...
<template>
  <div>
    <section class="relative mb-10 mt-5 flex justify-center items-center h-60">
      <img :src="subject?.img" class="absolute inset-0 object-cover object-center w-full h-full rounded-lg"
        alt="Subject cover" />
    ...
    <h3 class="font-bold text-4xl text-gray-800 my-6">
      <Button aria-label="Back" variant="link" @click="navigateTo('/review#review-progress')">
        <Icon class="me-3 text-gray-600 min-w-[22px]" name="uil:angle-left-b" size="22" />
      </Button>
      {{ subject?.title }}
    </h3>
    <div class="grid grid-cols-1 mt-16 gap-16">
      <Fieldset v-for="topic of subject?.topics" :key="topic.title" :legend="topic.title" pt:legend="font-bold text-xl"
        class="mb-6!">
        ...
        <FloatLabel class="mt-6" variant="on">
          ...
          <Textarea v-model="topic.comment" :id="topic.title + 'label'" :disabled="!topic.applicable" class="w-full"
            rows="5" cols="30" />
        </FloatLabel>
        ...
        <Fieldset :legend="topic.title + ' Checks'" toggleable class="mb-5! bg-slate-50!" pt:legend="bg-transparent!">
          ...
          <span v-for="(question, i) of topic.questions" :key="question.question">
            <ReviewQuestionRow :modelValue="question" @update:modelValue="topic.questions[i] = $event" class="my-3" />
          </span>
        </Fieldset>
      </Fieldset>
    </div>
  ...
  <div class="mt-16 w-full flex justify-center">
    <Button v-if="previousSubject" variant="link" severity="secondary"
      @click="navigateTo('/review/' + previousSubject.key)">
      <Icon class="text-gray-600 min-w-[22px]" name="uil:angle-left-b" size="22" />
      {{ previousSubject.title }}
    </Button>
    <Button v-if="nextSubject" variant="link" severity="secondary" @click="navigateTo('/review/' + nextSubject.key)">
      {{ nextSubject.title }}
      <Icon class="text-gray-600 min-w-[22px]" name="uil:angle-right-b" size="22" />
    </Button>
    <Button v-else variant="link" severity="secondary" @click="navigateTo('/score')">
      Go to score
      <Icon class="me-3 text-gray-600 min-w-[22px]" name="uil:angle-right-b" size="22" />
    </Button>
  </div>
</template>
