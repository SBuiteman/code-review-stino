...
<template>
  <Dialog v-model:visible="visible" maximizable modal :header="`Topic: ${selectedSubject?.title}`"
    :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
    <Transition enter-active-class="transition-opacity duration-300" enter-from-class="opacity-0"
      enter-leave-class="opacity-100" leave-active-class="transition-opacity duration-300"
      ...
      <section :key="selectedTopic?.description" class="flex flex-col">
        <div v-html="selectedTopic?.description"
    ...
    <Transition enter-active-class="transition-opacity duration-300" enter-from-class="opacity-0"
      ...
      <section :key="selectedQuestion.question" v-if="selectedQuestion" class="mt-2">
        ...
        <BaseRateSelect v-if="selectedQuestion.questionType === 'rating'" v-model="selectedQuestion.score"
          class="w-full! mt-3" />
        <BaseTrueFalse v-if="selectedQuestion.questionType === 'trueFalse'" v-model="selectedQuestion.score"
